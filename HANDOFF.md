...
- ~~手機直排版面再調（觸控按鈕位置）~~（v5 改為左搖桿+右技能鍵，含窄螢幕版面）
...
| `js/ui.js` | UI：畫面切換、選單生成（角色/盃賽/賽道卡片）、HUD、小地圖繪製、觸控（左手類比搖桿 `_initJoystick` + 右手道具/甩尾鍵）、墨漬/閃白/速度線 overlay。HUD 用 `_setText/_setHtml` 快取避免每幀重繪 |
